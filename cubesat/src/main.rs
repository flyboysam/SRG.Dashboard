//! CubeSat-SIM Telemetry API — for Raspberry Pi
//!
//! Reads telem.txt from the CubeSat simulator and serves JSON API.
//! Deploy to Pi, run from /home/pi or set TELEM_FILE.
...
const TELEM_FILE_PI: &str = "/home/pi/CubeSatSim/telem.txt";
...
async fn main() {
    ...
    let telem_file = std::env::var("TELEM_FILE")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from(TELEM_FILE_PI));
    ...
    let app = Router::new()
        ...
        .route("/api/users/delete", post(post_users_delete))
    ...
    println!(
        r#"
╔══════════════════════════════════════════════════════╗
║  CUBESAT-SIM  TELEMETRY API (Rust)                   ║
